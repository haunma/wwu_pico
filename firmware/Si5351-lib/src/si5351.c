...
int Si5351_WriteRegister(uint8_t reg_address, uint8_t reg_data)
{
  return g_i2c.Write(DI2C::ADDR_SI5351, reg_address, reg_data);
}

uint8_t Si5351_ReadRegister(uint8_t reg_address)
{
	return g_i2c.Read(DI2C::ADDR_SI5351, reg_address);
}
...
void Si5351_OSCConfig(Si5351_ConfigTypeDef *Si5351_ConfigStruct)
{
	...
	tmp = Si5351_ReadRegister(REG_XTAL_CL);
	...
	Si5351_WriteRegister(REG_XTAL_CL, tmp);

	//set CLKIN pre-divider
	tmp = Si5351_ReadRegister(REG_CLKIN_DIV);
	tmp &= ~CLKIN_MASK;
	tmp |= CLKIN_MASK & Si5351_ConfigStruct->OSC.CLKIN_Div;
	Si5351_WriteRegister(REG_CLKIN_DIV, tmp);

	//set fanout of XO, MS0, MS4 and CLKIN - should be always on unless you
	//need to reduce power consumption
	tmp = Si5351_ReadRegister(REG_FANOUT_EN);
	...
	Si5351_WriteRegister(REG_FANOUT_EN, tmp);
	...
	Si5351_WriteRegister(REG_VCXO_PARAM_0_7, tmp);
	tmp = (uint8_t)(VCXO_Param>>8);
	Si5351_WriteRegister(REG_VCXO_PARAM_8_15, tmp);
	tmp = (uint8_t)((VCXO_Param>>16) & VCXO_PARAM_16_21_MASK);
	Si5351_WriteRegister(REG_VCXO_PARAM_16_21, tmp);
}

EnableState Si5351_CheckStatusBit(Si5351_StatusBitTypeDef StatusBit)
{
	uint8_t tmp;

	tmp = Si5351_ReadRegister(REG_DEV_STATUS);
	tmp &= StatusBit;
	return (EnableState) tmp;
}

EnableState Si5351_CheckStickyBit(Si5351_StatusBitTypeDef StatusBit)
{
	uint8_t tmp;

	tmp = Si5351_ReadRegister(REG_DEV_STICKY);
	tmp &= StatusBit;
	return (EnableState) tmp;
}
...
{
	uint8_t tmp;
	tmp = Si5351_ReadRegister(REG_INT_MASK);
	...
	Si5351_WriteRegister(REG_INT_MASK, tmp);
}

void Si5351_ClearStickyBit(Si5351_StatusBitTypeDef StatusBit)
{
	uint8_t tmp;

	tmp = Si5351_ReadRegister(REG_DEV_STICKY);
	tmp &= ~StatusBit;
	Si5351_WriteRegister(REG_DEV_STICKY, tmp);
}
...
void Si5351_PLLConfig(Si5351_ConfigTypeDef *Si5351_ConfigStruct, Si5351_PLLChannelTypeDef PLL_Channel)
{
	...
	{
		tmp = Si5351_ReadRegister(REG_FB_INT + PLL_Channel);
		tmp &= ~FB_INT_MASK;
		Si5351_WriteRegister(REG_FB_INT + PLL_Channel, tmp);
	}
	...
	tmp = (uint8_t) MSN_P1;
	Si5351_WriteRegister(REG_MSN_P1_0_7 + 8 * PLL_Channel, tmp);
	tmp = (uint8_t) (MSN_P1 >> 8);
	Si5351_WriteRegister(REG_MSN_P1_8_15 + 8 * PLL_Channel, tmp);
	tmp = (uint8_t) (MSN_P1_16_17_MASK & (MSN_P1 >> 16));
	Si5351_WriteRegister(REG_MSN_P1_16_17 + 8 * PLL_Channel, tmp);

	tmp = (uint8_t) MSN_P2;
	Si5351_WriteRegister(REG_MSN_P2_0_7 + 8 * PLL_Channel, tmp);
	tmp = (uint8_t) (MSN_P2 >> 8);
	Si5351_WriteRegister(REG_MSN_P2_8_15 + 8 * PLL_Channel, tmp);
	tmp = Si5351_ReadRegister(REG_MSN_P2_16_19);
	tmp &= ~MSN_P2_16_19_MASK;
	tmp |= (uint8_t) (MSN_P2_16_19_MASK & (MSN_P2 >> 16));
	Si5351_WriteRegister(REG_MSN_P2_16_19 + 8 * PLL_Channel, tmp);

	tmp = (uint8_t) MSN_P3;
	Si5351_WriteRegister(REG_MSN_P3_0_7 + 8 * PLL_Channel, tmp);
	tmp = (uint8_t) (MSN_P3 >> 8);
	Si5351_WriteRegister(REG_MSN_P3_8_15 + 8 * PLL_Channel, tmp);
	tmp = Si5351_ReadRegister(REG_MSN_P3_16_19);
	tmp &= ~MSN_P3_16_19_MASK;
	tmp |= (uint8_t) (MSN_P3_16_19_MASK & ((MSN_P3 >> 16) << 4));
	Si5351_WriteRegister(REG_MSN_P3_16_19 + 8 * PLL_Channel, tmp);
...
void Si5351_PLLReset(Si5351_PLLChannelTypeDef PLL_Channel)
{
	uint8_t tmp;

	//reset PLL
	tmp = Si5351_ReadRegister(REG_PLL_RESET);
	...
	Si5351_WriteRegister(REG_PLL_RESET, tmp);
}

void Si5351_PLLSimultaneousReset()
{
	uint8_t tmp;
...
void Si5351_SSConfig(Si5351_ConfigTypeDef *Si5351_ConfigStruct)
{
	...
	{
		tmp = Si5351_ReadRegister(REG_SSC_EN);
		tmp &= ~SSC_EN_MASK;
		Si5351_WriteRegister(REG_SSC_EN, tmp);
	}

	//set default value of SS_NCLK - spread spectrum reserved register
	tmp = Si5351_ReadRegister(REG_SS_NCLK);
	tmp &= ~SS_NCLK_MASK;
	tmp |= SS_NCLK_MASK & (Si5351_ConfigStruct->SS.SS_NCLK);
	Si5351_WriteRegister(REG_SS_NCLK, tmp);

	//set SS mode
	tmp = Si5351_ReadRegister(REG_SSC_MODE);
	tmp &= ~SSC_MODE_MASK;
	tmp |= SSC_MODE_MASK & Si5351_ConfigStruct->SS.SS_Mode;
	Si5351_WriteRegister(REG_SSC_MODE, tmp);
	...
	//set SSUDP parameter
	tmp = (uint8_t) SSUDP;
	Si5351_WriteRegister(REG_SSUDP_0_7, tmp);
	tmp = Si5351_ReadRegister(REG_SSUDP_8_11);
	tmp &= ~SSUDP_8_11_MASK;
	tmp |= (uint8_t) (SSUDP_8_11_MASK & ((SSUDP >> 8) << 4));
	Si5351_WriteRegister(REG_SSUDP_8_11, tmp);
	...
	tmp = (uint8_t) SSUP_P1;
	Si5351_WriteRegister(REG_SSUP_P1_0_7, tmp);
	tmp = Si5351_ReadRegister(REG_SSUP_P1_8_11);
	tmp &= ~SSUP_P1_8_11_MASK;
	tmp |= (uint8_t)(SSUP_P1_8_11_MASK & (SSUP_P1 >> 8));
	Si5351_WriteRegister(REG_SSUP_P1_8_11, tmp);

	//write SSUP parameter P2
	tmp = (uint8_t) SSUP_P2;
	Si5351_WriteRegister(REG_SSUP_P2_0_7, tmp);
	tmp = Si5351_ReadRegister(REG_SSUP_P2_8_14);
	tmp &= ~SSUP_P2_8_14_MASK;
	tmp |= (uint8_t)(SSUP_P2_8_14_MASK & (SSUP_P2 >> 8));
	Si5351_WriteRegister(REG_SSUP_P2_8_14, tmp);

	//write SSUP parameter P3
	tmp = (uint8_t) SSUP_P3;
	Si5351_WriteRegister(REG_SSUP_P3_0_7, tmp);
	tmp = Si5351_ReadRegister(REG_SSUP_P3_8_14);
	tmp &= ~SSUP_P3_8_14_MASK;
	tmp |= (uint8_t)(SSUP_P3_8_14_MASK & (SSUP_P3 >> 8));
	Si5351_WriteRegister(REG_SSUP_P3_8_14, tmp);

	//write SSDN parameter P1
	tmp = (uint8_t) SSDN_P1;
	Si5351_WriteRegister(REG_SSDN_P1_0_7, tmp);
	tmp = Si5351_ReadRegister(REG_SSDN_P1_8_11);
	tmp &= ~SSDN_P1_8_11_MASK;
	tmp |= (uint8_t)(SSDN_P1_8_11_MASK & (SSDN_P1 >> 8));
	Si5351_WriteRegister(REG_SSDN_P1_8_11, tmp);

	//write SSDN parameter P2
	tmp = (uint8_t) SSDN_P2;
	Si5351_WriteRegister(REG_SSDN_P2_0_7, tmp);
	tmp = Si5351_ReadRegister(REG_SSDN_P2_8_14);
	tmp &= ~SSDN_P2_8_14_MASK;
	tmp |= (uint8_t)(SSDN_P2_8_14_MASK & (SSDN_P2 >> 8));
	Si5351_WriteRegister(REG_SSDN_P2_8_14, tmp);

	//write SSDN parameter P3
	tmp = (uint8_t) SSDN_P3;
	Si5351_WriteRegister(REG_SSDN_P3_0_7, tmp);
	tmp = Si5351_ReadRegister(REG_SSDN_P3_8_14);
	tmp &= ~SSDN_P3_8_14_MASK;
	tmp |= (uint8_t)(SSDN_P3_8_14_MASK & (SSDN_P3 >> 8));
	Si5351_WriteRegister(REG_SSDN_P3_8_14, tmp);
...
void Si5351_MSConfig(Si5351_ConfigTypeDef *Si5351_ConfigStruct, Si5351_MSChannelTypeDef MS_Channel)
{
	...
	tmp = Si5351_ReadRegister(REG_MS_SRC + MS_Channel);
	tmp &= ~MS_SRC_MASK;
	if (Si5351_ConfigStruct->MS[MS_Channel].MS_Clock_Source == MS_Clock_Source_PLLB)
	{
		tmp |= MS_SRC_MASK;
	}
	Si5351_WriteRegister(REG_MS_SRC + MS_Channel, tmp);
	...
	if (MS_Channel <= MS5) //configuration is simpler for MS6 and 7 since they are integer-only
	{
		...
		{
			tmp = Si5351_ReadRegister(REG_MS_DIVBY4 + 8 * MS_Channel);
			tmp &= ~MS_DIVBY4_MASK;
			Si5351_WriteRegister(REG_MS_DIVBY4 + 8 * MS_Channel, tmp);
		}
		...
		{
			tmp = Si5351_ReadRegister(REG_MS_INT + MS_Channel);
			tmp &= ~MS_INT_MASK;
			Si5351_WriteRegister(REG_MS_INT + MS_Channel, tmp);
		}
		...
		tmp = (uint8_t) MS_P1;
		Si5351_WriteRegister(REG_MS_P1_0_7 + 8 * MS_Channel, tmp);
		tmp = (uint8_t) (MS_P1 >> 8);
		Si5351_WriteRegister(REG_MS_P1_8_15 + 8 * MS_Channel, tmp);
		tmp = Si5351_ReadRegister(REG_MS_P1_16_17);
		tmp &= ~MS_P1_16_17_MASK;
		tmp |= (uint8_t) (MS_P1_16_17_MASK & (MS_P1 >> 16));
		Si5351_WriteRegister(REG_MS_P1_16_17 + 8 * MS_Channel, tmp);

		tmp = (uint8_t) MS_P2;
		Si5351_WriteRegister(REG_MS_P2_0_7 + 8 * MS_Channel, tmp);
		tmp = (uint8_t) (MS_P2 >> 8);
		Si5351_WriteRegister(REG_MS_P2_8_15 + 8 * MS_Channel, tmp);
		Si5351_ReadRegister(REG_MS_P2_16_19 + 8 * MS_Channel);
		tmp &= ~MS_P2_16_19_MASK;
		tmp |= (uint8_t) (MS_P2_16_19_MASK & (MS_P2 >> 16));
		Si5351_WriteRegister(REG_MS_P2_16_19 + 8 * MS_Channel, tmp);

		tmp = (uint8_t) MS_P3;
		Si5351_WriteRegister(REG_MS_P3_0_7 + 8 * MS_Channel, tmp);
		tmp = (uint8_t) (MS_P3 >> 8);
		Si5351_WriteRegister(REG_MS_P3_8_15 + 8 * MS_Channel, tmp);
		tmp = Si5351_ReadRegister(REG_MS_P3_16_19 + 8 * MS_Channel);
		tmp &= ~MS_P3_16_19_MASK;
		tmp |= (uint8_t) (MS_P3_16_19_MASK & ((MS_P3 >> 16) << 4));
		Si5351_WriteRegister(REG_MS_P3_16_19 + 8 * MS_Channel, tmp);
		...
		Si5351_WriteRegister(REG_MS67_P1 + (MS_Channel - MS6), (uint8_t)(Si5351_ConfigStruct->MS[MS_Channel].MS_Divider_Integer));
...
void Si5351_CLKPowerCmd(Si5351_ConfigTypeDef *Si5351_ConfigStruct, Si5351_CLKChannelTypeDef CLK_Channel)
{
	...
	tmp = Si5351_ReadRegister(REG_CLK_DIS_STATE + (CLK_Channel >> 2)); //increment the address by 1 if CLKx>=CLK4
	...
	Si5351_WriteRegister(REG_CLK_DIS_STATE + (CLK_Channel >> 2), tmp);

	//set OEB pin
	tmp = Si5351_ReadRegister(REG_CLK_OEB);
	...
	if (Si5351_ConfigStruct->CLK[CLK_Channel].CLK_Enable == OFF) //disable clock
	{
		...
		tmp = Si5351_ReadRegister(REG_CLK_EN);
		tmp |= 1 << CLK_Channel;
		Si5351_WriteRegister(REG_CLK_EN, tmp);
	}

	if (Si5351_ConfigStruct->CLK[CLK_Channel].CLK_PowerDown == ON) //power down clock
	{
		//power down output driver
		tmp = Si5351_ReadRegister(REG_CLK_PDN + CLK_Channel);
		tmp |= CLK_PDN_MASK;
		Si5351_WriteRegister(REG_CLK_PDN + CLK_Channel, tmp);
	}
...
void Si5351_CLKConfig(Si5351_ConfigTypeDef *Si5351_ConfigStruct, Si5351_CLKChannelTypeDef CLK_Channel)
{
	...
	tmp = Si5351_ReadRegister(REG_CLK_SRC + CLK_Channel);
	tmp &= ~CLK_SRC_MASK;
	tmp |= CLK_SRC_MASK & Si5351_ConfigStruct->CLK[CLK_Channel].CLK_Clock_Source;
	Si5351_WriteRegister(REG_CLK_SRC + CLK_Channel, tmp);

	//set CLK inversion
	tmp = Si5351_ReadRegister(REG_CLK_INV + CLK_Channel);
	tmp &= ~CLK_INV_MASK;
	if (Si5351_ConfigStruct->CLK[CLK_Channel].CLK_Invert == ON)
	{
		tmp |= CLK_INV_MASK;
	}
	Si5351_WriteRegister(REG_CLK_INV + CLK_Channel, tmp);

	//set CLK current drive
	tmp = Si5351_ReadRegister(REG_CLK_IDRV + CLK_Channel);
	tmp &= ~CLK_IDRV_MASK;
	tmp |= CLK_IDRV_MASK & Si5351_ConfigStruct->CLK[CLK_Channel].CLK_I_Drv;
	Si5351_WriteRegister(REG_CLK_IDRV + CLK_Channel, tmp);
	...
	if (CLK_Channel <= CLK5) //CLK6 and 7 are integer only, which causes several limitations
	{
		...
		Si5351_WriteRegister(REG_CLK_PHOFF + CLK_Channel, tmp);
		//set Rx divider
		tmp = Si5351_ReadRegister(REG_CLK_R_DIV + CLK_Channel * CLK_R_DIV_STEP);
		tmp &= ~CLK_R_DIV_MASK;
		tmp |= CLK_R_DIV_MASK & (Si5351_ConfigStruct->CLK[CLK_Channel].CLK_R_Div);
		Si5351_WriteRegister(REG_CLK_R_DIV + CLK_Channel * CLK_R_DIV_STEP, tmp);
	} else {
		...
		tmp = Si5351_ReadRegister(REG_CLK_R67_DIV);
		tmp &= ~tmp_mask;
		tmp |= tmp_mask & ((Si5351_ConfigStruct->CLK[CLK_Channel].CLK_R_Div >> 4) << ((CLK_Channel-CLK6) << 2));
		Si5351_WriteRegister(REG_CLK_R67_DIV, tmp);
	}
}
...
int Si5351_Init(Si5351_ConfigTypeDef *Si5351_ConfigStruct)
{
	...
	while (Si5351_CheckStatusBit(StatusBit_SysInit))
	{
		timeout--;
	...
	for (i=CLK0; i<=CLK7; i++)
	{
		Si5351_CLKPowerCmd(Si5351_ConfigStruct, (Si5351_CLKChannelTypeDef) i);
	}
	...
	{
		Si5351_PLLConfig(Si5351_ConfigStruct, (Si5351_PLLChannelTypeDef) i);
		Si5351_PLLReset((Si5351_PLLChannelTypeDef) i);
	}
	...
	for (i=MS0; i<=MS7; i++)
	{
		Si5351_MSConfig(Si5351_ConfigStruct, (Si5351_MSChannelTypeDef) i);
	}

	//configure outputs
	for (i=CLK0; i<=CLK7; i++)
	{
		Si5351_CLKConfig(Si5351_ConfigStruct, (Si5351_CLKChannelTypeDef) i);
	}

	//wait for PLLs to lock
	while (Si5351_CheckStatusBit((Si5351_StatusBitTypeDef) (StatusBit_SysInit | StatusBit_PLLA | StatusBit_PLLB)))
	{
		timeout--;
	...
	//clear sticky bits
	Si5351_ClearStickyBit((Si5351_StatusBitTypeDef) (StatusBit_SysInit | StatusBit_PLLA | StatusBit_PLLB));

	if (Si5351_ConfigStruct->f_CLKIN != 0) //if CLKIN used, check it as well
	{
		while (Si5351_CheckStatusBit(StatusBit_CLKIN))
		{
			timeout--;
			if (timeout==0) return 1; //return 1 if initialization timed out
		}
		//clear CLKIN sticky bit
		Si5351_ClearStickyBit(StatusBit_CLKIN);
	}

	if (Si5351_ConfigStruct->f_XTAL != 0) //if XTAL used, check it as well
	{
		while (Si5351_CheckStatusBit(StatusBit_XTAL))
		{
			timeout--;
			if (timeout==0) return 1; //return 1 if initialization timed out
		}
		//clear XTAL sticky bit
		Si5351_ClearStickyBit(StatusBit_XTAL);
	}
